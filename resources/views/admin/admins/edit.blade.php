        ...
        <div class="row">
            ...
            <div class="col-sm-3">
                <div class="admin-avatar-img">
                    <img id="avatar-admin-id" src="{{ $admin->avatar }}" alt=""
                    ...
                    @can('is-super-admin')
                        <div class="form-group col-sm-6">
                            ...
                            <select class="form-control" name="role" value="">
                                @foreach (config('user.roles') as $key => $role)
                                    <option value="{{ $key }}" @if ($key ==
                                        $admin->id) selected @endif>
                                        {{ $role }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="form-group col-sm-6">
                            ...
                            <select class="form-control" name="lock">
                                <option value="0" {{ $admin->lock ==0 ??'selected' }}>Unlock
                                </option>
                                <option value="1" {{ $admin->lock ==1 ??'selected' }}>Locked
                                </option>
                            </select>
                        </div>
                    @else
